...
if ($tcek > 0)
{
  if (($nip <> "semua") and (!empty($nip)))
  {
	  ...
	  mst_jamkerja c where a.kode_jab = b.kode_jab and nip = '$nip'")or die(mysql_error());
	  $t = mysql_fetch_array($q);
?>
</p>
<table width="600" border="0" align="center" cellpadding="0" cellspacing="2">
  <tr>
    <td width="445"><? echo "<a href='claporan.php?bulan=$bulan&tahun=$tahun&nip=$nip' class='versicetak' target='_blank'>"; ?>Versi Cetak</a> </td>
  </tr>
</table>
<table width="600" border="0" align="center" cellpadding="0" cellspacing="2">
  <tr>
    <td class="bgform"><table width="100%" border="0" cellspacing="2" cellpadding="0">
      ...
      <tr>
        ...
        <td><? echo $t[nama_jabatan]; ?></td>
      </tr>
<!--
      <tr>
        <td class="labelform">SHIP / Jam Kerja </td>
        <td><? // echo $t[nama_ship]." / ".$t[jam_masuk]." s/d ".$t[jam_pulang]; ?></td>
      </tr>
-->
      <tr>
        <td>&nbsp;</td>
...
<?
   ...
   for ($hr=1;$hr<=$jumhari;$hr++)
   {
	...
	$qabsen2 = mysql_query("SELECT (count(tanggal)*TIMEDIFF(pulang, masuk)) as jmlh FROM t_absensi where month(tanggal)='$bulan' and year(tanggal)='$tahun' and nip='$t[nip]'")or die(mysql_error());	  					   
...
//	print_r($tabsen['jumlah']);
	...
	$qcuti = mysql_query("select count(nip) as tgl_awal,nip,keperluan, tanggal_awal,tanggal_akhir, jenis_cuti from t_cuti where nip = '$t[nip]'
							and '$cektanggal' between date(tanggal_awal) and date(tanggal_akhir) group by nip")or die (mysql_error());
	...
	if (empty($jampulang)) { $jampulang = "17:30:00"; }
	if (($nhari == 0) or ($adalibur>0))
	{
	  echo "<tr bgcolor='red'>";
	} else
	if (empty($tabsen[tanggal]))
	{
	  echo "<tr bgcolor='yellow'>";
	  $tidakmasuk++;
	} else
...
?>
        <td class="tabelisi" align="center"><? echo $hari; ?></td>
        <td class="tabelisi" align="center"><? echo $tanggal; ?></td>
        <td class="tabelisi" align="center"><? echo $tabsen[masuk]; ?></td>
        <td class="tabelisi" align="center"><? echo $tabsen[pulang]; ?></td>
        <td class="tabelisi" align="center"><? echo $tabsen[jumlah]; 
			$j = $j + $tabsen[jumlah];
		?></td>
        <td class="tabelisi"><? echo $keterangan; ?></td>
      </tr>
<? } ?>
      <tr>
        <td>&nbsp;</td>
  ...
  <tr>
    <td width="304" class="bgform"><table width="100%" border="0" align="center" cellpadding="0" cellspacing="2">
          ...
          <tr>
            ...
            <td class="tabelisi"><? echo $tidakmasuk; ?></td>
          </tr>
          <tr>
            <td class="tabelisi">Jumlah Kehadiran (hari) </td>
            <td class="tabelisi"><? echo $totalmasuk; ?></td>
          </tr>
          <tr>
            <td class="tabelisi">Jumlah Ijin (hari) </td>
            <td class="tabelisi"><? echo $totalijin; ?></td>
          </tr>
          <tr>
            <td class="tabelisi">Jumlah Cuti (hari) </td>
            <td class="tabelisi"><? echo $total_cuti; ?></td>
          </tr>
          <tr>
            <td class="tabelisi">Jumlah Lembur (Jam)</td>
            <td class="tabelisi"><? echo $lem; ?></td>
          </tr>
    </table></td>
  </tr>
</table>
