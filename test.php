...
<table>
<?php
$qkar = mysql_query('select mst_karyawan.* from mst_karyawan') or exit(mysql_error());
  $nomor = 1;
  ...
  $nip = $tkar[nip];
    $sql = mysql_query("select mst_karyawan.nama_lengkap,t_absensi.tanggal,t_absensi.masuk,
						   ...
						   group by mst_karyawan.nip") or exit(mysql_error());
 ...
 <tr class="tabelisi">
    <td class="tabelisi"><?php echo $nomor++; ?></td>
    <td class="tabelisi"><?php echo $tabsen[nama_lengkap]; ?></td>
	<?php
    $totjam[] = $tabsen[jumlah];
                               $totaljam = total_jam($totjam); ?>
	 <td class="tabelisi" align="center"><?php echo $totaljam; ?>
	</td>
</tr>
<?php
                           }
